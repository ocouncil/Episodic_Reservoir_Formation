...
def numbers():
    ...
    output = random.choices(sample, weights=(6, 7, 10, 11, 13, 15, 18, 21), k=22)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.06, 0.07, 0.10, 0.11, 0.13, 0.15, 0.18, 0.21]
    early_seqs = 22
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230919_CAP277_ChiSquare_108-week_halflife_MidPoint_Bins_Expected_Seqs_Rounded.csv', 'w') as f:
    writer = csv.writer(f)
