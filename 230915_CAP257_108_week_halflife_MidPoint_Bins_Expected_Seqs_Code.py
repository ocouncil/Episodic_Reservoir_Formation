...
def numbers():
    ...
    output = random.choices(sample, weights=(6, 7, 9, 11, 14, 16, 20, 17), k=27)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.06, 0.07, 0.09, 0.11, 0.14, 0.16, 0.2, 0.17]
    early_seqs = 27
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230919_CAP257_ChiSquare_MidPoint_Bins_108_week_Halflife_Expected_Seqs_Rounded.csv', 'w') as f:
    writer = csv.writer(f)
