...
def numbers():
    ...
    output = random.choices(sample, weights=(5, 8, 12, 11, 10, 11, 18, 25), k=62)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.05, 0.08, 0.12, 0.11, 0.10, 0.11, 0.18, 0.25]
    early_seqs = 62
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230919_CAP287_ChiSquare_108_week_Halflife_MidPoint_Bins_Expected_Seqs_Rounded.csv', 'w') as f:
    writer = csv.writer(f)
