...
def numbers():
    ...
    output = random.choices(sample, weights=(7, 5, 7, 9, 12, 16, 22, 22), k=27)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.07, 0.05, 0.07, 0.09, 0.12, 0.16, 0.22, 0.22]
    early_seqs = 27
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240822_CAP257_ChiSquare_PreART_Decay_191week_halflife_Rounded_Midpoint_Bin.csv', 'w') as f:
    writer = csv.writer(f)
