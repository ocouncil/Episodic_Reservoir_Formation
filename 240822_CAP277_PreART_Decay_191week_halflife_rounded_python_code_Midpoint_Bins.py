...
def numbers():
    ...
    output = random.choices(sample, weights=(8, 9, 11, 12, 13, 14, 16, 17), k=22)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.08, 0.09, 0.11, 0.12, 0.13, 0.14, 0.16, 0.17]
    early_seqs = 22
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240822_CAP277_ChiSquare_PreART_Decay_191week_halflife_Rounded_Midpoint_Bin.csv', 'w') as f:
    writer = csv.writer(f)
