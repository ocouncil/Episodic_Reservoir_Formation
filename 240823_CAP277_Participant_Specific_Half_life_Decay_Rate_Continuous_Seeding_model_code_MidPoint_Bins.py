...
import array
#CAP277
#using participant-specific pre-ART half-life for decay
...
def numbers():
    ...
    output = random.choices(sample, weights=(5, 6, 9, 10, 12, 16, 19, 23), k=22)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.05, 0.06, 0.09, 0.1, 0.12, 0.16, 0.19, 0.23]
    early_seqs = 22
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240823_CAP277_ChiSquare_ParticipantSpecific_Decay_Rounded_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
