...
import statistics
import array
#CAP188 expected seqs rounded to nearest whole number
#midpoint bins
#108-week_halflife
...
def numbers():
    ...
    output = random.choices(sample, weights=(7, 7, 7, 11, 15, 15, 18, 20), k=61)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.07, 0.07, 0.07, 0.11, 0.15, 0.15, 0.18, 0.20]
    early_seqs = 61
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230919_CAP188_ChiSquare_108_week_Halflife_Expected_Seqs_Round_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
