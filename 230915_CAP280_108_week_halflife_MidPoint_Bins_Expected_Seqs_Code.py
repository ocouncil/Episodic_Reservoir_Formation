...
def numbers():
    ...
    output = random.choices(sample, weights=(4, 5, 7, 8, 12, 15, 14, 12, 17, 7), k=39)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.04, 0.05, 0.07, 0.08, 0.12, 0.15, 0.14, 0.12, 0.17, 0.07]
    early_seqs = 39
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230919_CAP280_ChiSquare_108week_halflife_MidPoint_Bins_Expected_Seqs_Rounded.csv', 'w') as f:
    writer = csv.writer(f)
