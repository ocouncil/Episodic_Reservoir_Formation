...
#CAP302, participant-specific decay, negative binomial regression estimated
...
def numbers():
    ...
    bin5 = 0
    sample = [1, 2, 3, 4, 5]
    output = random.choices(sample, weights=(7, 13, 23, 30, 28), k=32)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.07, 0.13, 0.23, 0.30, 0.28]
    early_seqs = 32
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240823_CAP302_Participant_Specific_Decay_Rounded_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
