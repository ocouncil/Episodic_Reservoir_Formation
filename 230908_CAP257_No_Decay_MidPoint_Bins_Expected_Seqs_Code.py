...
def numbers():
    ...
    output = random.choices(sample, weights=(12, 12, 12, 13, 14, 13, 14, 10), k=27)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.12, 0.12, 0.12, 0.13, 0.14, 0.13, 0.14, 0.10]
    early_seqs = 27
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230915_CAP257_ChiSquare_MidPoint_Bins_No_Decay_Expected_Seqs_Rounded.csv', 'w') as f:
    writer = csv.writer(f)
