...
def numbers():
    ...
    output = random.choices(sample, weights=(4, 4, 5, 9, 9, 8, 13, 12, 11, 12, 12), k=39)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.04, 0.04, 0.05, 0.09, 0.09, 0.08, 0.13, 0.12, 0.11, 0.12, 0.12]
    early_seqs = 39
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240822_CAP244_ChiSquare_Pre_ART_Decay_191_week_halflife_Rounded_Midpoint_bin.csv', 'w') as f:
    writer = csv.writer(f)
