...
def numbers():
    ...
    output = random.choices(sample, weights=(3, 3, 3, 7, 7, 7, 13, 13, 13, 15, 17), k=43)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.03, 0.03, 0.03, 0.07, 0.07, 0.07, 0.13, 0.13, 0.13, 0.15, 0.17]
    early_seqs = 43
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230919_CAP244_ChiSquare_108_week_halflife_Expected_Seqs_Rounded_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
