...
def numbers():
    ...
    output = random.choices(sample, weights=(7, 9, 15, 12, 9, 11, 16, 21), k=61)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.07, 0.09, 0.15, 0.12, 0.09, 0.11, 0.16, 0.21]
    early_seqs = 61
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240822_CAP287_ChiSquare_PreART_Decay_191week_halflife_Rounded_Midpoint_Bin.csv', 'w') as f:
    writer = csv.writer(f)
