...
import statistics
import array
#CAP287, participant-specific decay, negative binomial regression estimated
...
def numbers():
    ...
    output = random.choices(sample, weights=(7, 10, 15, 12, 9, 11, 15, 20), k=61)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.07, 0.1, 0.15, 0.12, 0.09, 0.11, 0.15, 0.20]
    early_seqs = 61
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('240823_CAP287_Participant_Specific_Decay_Rounded_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
