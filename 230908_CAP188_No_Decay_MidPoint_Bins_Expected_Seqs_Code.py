...
import statistics
import array
#CAP188, no decay, expected seqs rounded to nearest whole number
#midpoint bins
...
def numbers():
    ...
    output = random.choices(sample, weights=(13, 12, 9, 13, 15, 13, 13, 12), k=61)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.13, 0.12, 0.09, 0.13, 0.15, 0.13, 0.13, 0.12]
    early_seqs = 61
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230908_CAP188_ChiSquare_No_Decay_Expected_Seqs_Round_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
