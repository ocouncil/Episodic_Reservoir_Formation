...
def numbers():
    ...
    output = random.choices(sample, weights=(8, 7, 7, 13, 11, 8, 12, 10, 8, 8, 8), k=43)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.08, 0.07, 0.07, 0.13, 0.11, 0.08, 0.12, 0.10, 0.08, 0.08, 0.08]
    early_seqs = 43
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230908_CAP244_ChiSquare_No_Decay_Expected_Seqs_Rounded_MidPoint_Bins.csv', 'w') as f:
    writer = csv.writer(f)
