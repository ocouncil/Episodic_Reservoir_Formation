...
def numbers():
    ...
    output = random.choices(sample, weights=(11, 12, 13, 13, 13, 13, 13, 13), k=22)
    for i in output:
        if i == 1:
            bin1 = bin1 + 1
        if i == 2:
    ...
    weights = [0.11, 0.12, 0.13, 0.13, 0.13, 0.13, 0.13, 0.13]
    early_seqs = 22
    expected_seqs = []
    obs_minus_expected = []
    for i in weights:
...
with open('230915_CAP277_ChiSquare_No_Decay_MidPoint_Bins_Expected_Seqs_Rounded.csv', 'w') as f:
    writer = csv.writer(f)
